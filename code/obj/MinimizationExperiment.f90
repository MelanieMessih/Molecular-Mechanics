...
contains
    ...
    subroutine RunMinimizationExperiment(inputFilename, outputFilename, T, r, tolerance, maxIter)
        character(len=20), intent(in)   :: inputFilename, outputFilename
        ...
        integer                         :: CH_BondCount, CC_BondCount
        integer                         :: CCC_Count, CCH_Count, HCH_Count
        integer                         :: CCCC_Count, CCCH_Count, HCCH_Count
        integer                         :: CC_nbCount, HC_nbCount, HH_nbCount
        integer                         :: iterTaken
        real(kind=8)                    :: i_stretchEnergy, i_bendEnergy, i_torsionEnergy, i_nonbondedEnergy, i_totalEnergy
        real(kind=8)                    :: f_stretchEnergy, f_bendEnergy, f_torsionEnergy, f_nonbondedEnergy, f_totalEnergy
        ...
        i_nonbondedEnergy = CalculateNonBondedEnergy(atoms, separationCount)
        i_totalEnergy = CalculateTotalEnergy(i_stretchEnergy, i_bendEnergy, i_torsionEnergy, i_nonbondedEnergy)
        ...
        f_nonbondedEnergy = CalculateNonBondedEnergy(atoms, separationCount)
        f_totalEnergy = CalculateTotalEnergy(f_stretchEnergy, f_bendEnergy, f_torsionEnergy, f_nonbondedEnergy)

        ! Write output energy values to a file
        call WriteOutput(outputFilename, inputFilename, CC_BondCount, CH_BondCount, &
                         CCC_Count, CCH_Count, HCH_Count, &
                         CCCC_Count, CCCH_Count, HCCH_Count, &
                         CC_nbCount, HC_nbCount, HH_nbCount, &
                         i_stretchEnergy, i_bendEnergy, i_torsionEnergy, i_nonbondedEnergy, i_totalEnergy, &
                         f_stretchEnergy, f_bendEnergy, f_torsionEnergy, f_nonbondedEnergy, f_totalEnergy, &
